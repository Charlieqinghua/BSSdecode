...
import templateBean.egcdr.EGCDRPaseDecodeBean;
import templateBean.egcdr.EGCDRPaseDecodeBeanDebug;
...
public class EGCDRMutiThreadGetSubFileBean extends Thread {
	...
	TmplatePaseDecodeBean scdrdecode=new EGCDRPaseDecodeBean(RowBeanlist,afdecode);
	FileOutResultAllFieldBean out=new FileOutResultAllFieldBean(DicTion.egcdrhead);
	String outdir=DicTion.outdir;
		
	public EGCDRMutiThreadGetSubFileBean(String outputdir ) {
		super();
		outdir=outputdir;
	}
	//使用工厂动态创建后需要修改构造函数
	public EGCDRMutiThreadGetSubFileBean() {
		super();
	}
...
//			}
			...
			try 
			{
				...
				scdrdecode.match(DicTion.egcdrdic);
	...
	public static  void  main(String args[]) throws Throwable
	{
		...
		for(int i=0;i<3;i++)
		{
			EGCDRMutiThreadGetSubFileBean onethread=new EGCDRMutiThreadGetSubFileBean(outdir);
			onethread.start();
		}
	}
