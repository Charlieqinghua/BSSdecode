...
import templateBean.FileOutResultAllFieldBeanGz;
...
import templateBean.zxscdr.ZXScdrTmplatePaseDecodeBean;
import templateBean.zxscdr.ZXScdrTmplatePaseDecodeBeanDebug;
...
import all.DicTion;
/*
 * 供魏进武使用的人口信息跟踪的系统zxscdr的话单
 * 
 * */

public class ZXSCDRDemoThreadFileBean extends Thread {
	...
	TmplatePaseDecodeBean basedecode=new ZXScdrTmplatePaseDecodeBean(RowBeanlist,afdecode);
	//FileOutResultAllFieldBean out=new FileOutResultAllFieldBean(DicTion.scdrhead);
	FileOutResultAllFieldBean out=new FileOutResultAllFieldBeanGz(Area.clound);
	String outdir=DicTion.outdir;
		
	public ZXSCDRDemoThreadFileBean(String outputdir ) {
		super();
		outdir=outputdir;
	}
	//使用工厂动态创建后需要修改构造函数
	public ZXSCDRDemoThreadFileBean() {
		super();
	}
...
//			}
			...
			try 
			{
				basedecode.readGzTobytes(f.getAbsolutePath());
				basedecode.match(DicTion.zxsdrdic);
				List<RowBean> wordlist=basedecode.getWordlist();
	...
	public static  void  main(String args[]) throws Throwable
	{
		...
		for(int i=0;i<3;i++)
		{
			ZXSCDRDemoThreadFileBean onethread=new ZXSCDRDemoThreadFileBean(outdir);
			onethread.start();
		}
	}
