 ...
 * @param args
 */
public class PGWPaseDecodeBeanDebug extends templateBean.TmplatePaseDecodeBean {
	...
	public PGWPaseDecodeBeanDebug(List<RowBean> rowBeanlist,
	...
	public int dealTLV(int starttag,int pos,int way,int arrnum)//处理完 tlv 返回 最后的位置
	{
		...
		if(taglen<0) 
		{
			...
			{
				taglen=HexString.ComputeTwoTagLengh(b,pos+2,pos+3);//0x82c3模式
				pos=pos+2;//跳过length
			}
			//添加判断的
			else if(taglen==b83)//0x82018a后面两个是长度
			{
				taglen=HexString.ComputeThreeTagLengh(b,pos+2,pos+4);//0x83c3模式
				pos=pos+3;//跳过length
			}
			else 
			{
...
//				
		...
		//if(!tag.equals("\r\n0xb482")) rowbean.add(word,arrnum);
	...
	public static void main(String[] args) {
		...
		TmplatePaseDecodeBean pase=new PGWPaseDecodeBeanDebug(RowBeanlist,afdecode);
