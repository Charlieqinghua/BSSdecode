...
/**
 * @param args
 * 与debug不同的地方是不需要 对 af 标签和 bf22 标签进行 标签校验内容输出
 ...
 */
public class EGCDRPaseDecodeBean extends templateBean.TmplatePaseDecodeBean {
	...
	public EGCDRPaseDecodeBean(List<RowBean> rowBeanlist,
	...
	public int dealTV(int starttag,int pos,int way,int arrnum)//处理完 tlv 返回 最后的位置
	{
		...
		switch(way)
		{
			...
			case 0://是ＴＶ也是开始ＲｏｗＢｅａｎ
			{
				...
				if(tag.equals(startflag))
				{		
					//不进行流量合并创建新的之前先合并流量  uplink 13,62 downlink 14,63
//					if(rowbean!=null)
//					{
//						mergeVolume(rowbean);
//					}
...
//					rowbean.add(word,arrnum);
	...
	public static void main(String[] args) {
		...
		TmplatePaseDecodeBean pase=new EGCDRPaseDecodeBean(RowBeanlist,afdecode);
